...
class AppLocalization {
  ...
  static AppLocalization? of(final BuildContext context) =>
      Localizations.of(context, AppLocalization);

  //to load json(language) from assets
  Future<void> loadJson() async {
    final String languageJsonName = locale.languageCode;
...
class _AppLocalizationDelegate extends LocalizationsDelegate<AppLocalization> {
  ...
  bool isSupported(final Locale locale) {
    ...
    return appLanguages
        .map(
          (final appLanguage) =>
              getLocaleFromLanguageCode(appLanguage.languageCode),
        )
        .toList()
        .contains(locale);
  }
