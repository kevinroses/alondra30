...
class StateItem extends State<ProductDetail> with TickerProviderStateMixin {
  ...
  final NumberFormat miles = NumberFormat('#,##0', 'es_MX');
  ...
  void _scrollListenercity() async {
  ...
  Future<void> getProFavIds() async {
    proIds1 = (await db.getMostFav())!;
    getMostFavPro();
  }

  Future<void> checkProId() async {
  ...
  List<Widget> getCityList(StateSetter setStater, AddressProvider data) {
  ...
  Future<void> callApi() async {
    ...
    if (isNetworkAvail) {
      ...
      if (context.read<CartProvider>().addressList[widget.index!].cityId !=
          '0') {
                        ...
                        Align(
                          ...
                          child: Row(
                            children: [
                              Expanded(
                                child: Text(
                                  'CHECK_PRODUCT_AVAILABILITY'.translate(
                                    context: context,
                                  ),
                        ...
                        context
                            ...
                            ? TextFormField(
                                ...
                                decoration: InputDecoration(
                                  ...
                                  hintText:
                                      'CITY_LBL'.translate(context: context),
                                  ...
                                  suffix: GestureDetector(
                                    ...
                                    child: Text(
                                      'Check'.translate(context: context),
                                      style: const TextStyle(
                                        color: colors.primary,
                                        fontFamily: 'ubuntu',
                                      ),
                            ...
                            : TextFormField(
                                ...
                                validator: (val) =>
                                    StringValidation.validatePincode(
                                  val!,
                                  'PIN_REQUIRED'.translate(context: context),
                                ),
                                onSaved: (String? value) {
  ...
  Future<void> addAndRemoveQty(
      String qty, int from, int totalLen, int itemCounter) async {
      ...
      if (isNetworkAvail) {
        ...
        if (context.read<UserProvider>().userId != '') {
                ...
                if (!error) {
                  ...
                  if (from) {
                    ...
                    if (cart is List) {
                      ...
                      // Forzar actualización del precio total
                      context.read<CartProvider>().notifyListeners();
                      
                      // Aplicar automáticamente descuento por montos si no hay ID de cliente
                      try {
                        final userProvider = context.read<UserProvider>();
                        final tieneIdCliente = userProvider.customerErpId != null && userProvider.customerErpId!.isNotEmpty;
                        
                        print('DEBUG ProductDetail: tieneIdCliente = $tieneIdCliente');
                        print('DEBUG ProductDetail: cartList.isNotEmpty = ${context.read<CartProvider>().cartList.isNotEmpty}');
                        
                        if (!tieneIdCliente && context.read<CartProvider>().cartList.isNotEmpty) {
                          final totalCarrito = context.read<CartProvider>().oriPrice;
                          final descuentoPorcentaje = calcularDescuento(totalCarrito);
                          
                          print('DEBUG ProductDetail: totalCarrito = $totalCarrito');
                          print('DEBUG ProductDetail: descuentoPorcentaje = ${descuentoPorcentaje * 100}%');
                          
                          // Aplicar descuento a todos los productos del carrito
                          for (var item in context.read<CartProvider>().cartList) {
                            ...
                            if (item.productList != null && item.productList!.isNotEmpty) {
                              ...
                              if (product.prVarientList != null && product.prVarientList!.isNotEmpty) {
                                ...
                                print('DEBUG ProductDetail: Item ${item.id} - Precio original: $precioOriginal, Precio con descuento: $precioConDescuento');
                              }
                            }
                          }
                          
                          // Recalcular precios totales con descuentos
                          context.read<CartProvider>().recalculateTotals();
                          
                          print('DEBUG ProductDetail: Después de aplicar descuento - oriPrice: ${context.read<CartProvider>().oriPrice}, totalPrice: ${context.read<CartProvider>().totalPrice}');
                        }
                      } catch (e) {
                        print('Error aplicando descuento automático en ProductDetail: $e');
                      }
                    }
                    if (intent) {
                  ...
                  if (msg !=
                    ...
                    // Manejo específico para error de cantidad excedida
                    if (msg != null && msg.toLowerCase().contains('quantity exceeds')) {
                      setSnackbar(
                        'Has alcanzado la cantidad máxima permitida para este producto. Por favor, reduce la cantidad.',
                        context,
                      );
                    } else {
          ...
          if (singleSellerOrderSystem) {
            ...
            setSnackbar('PRODUCT_ADDED_TO_CART_LBL'.translate(context: context),
                context);
            Future.delayed(const Duration(milliseconds: 100)).then(
              (_) async {
                if (from && intent) {
  ...
  void _setFav(int index, int from) async {
    try {
      isNetworkAvail = await isNetworkAvailable();
      if (isNetworkAvail) {
        try {
  ...
  void _removeFav(int index, int from) async {
    try {
      isNetworkAvail = await isNetworkAvailable();
      if (isNetworkAvail) {
        try {
  ...
  Widget getSilverAppBar(Product model) {
    return SliverAppBar(
  ...
  Widget _showContent() {
    final stockValue = widget.model!.stock is num
        ? widget.model!.stock
        : num.tryParse(widget.model!.stock.toString()) ?? 0;
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: <Widget>[
        Expanded(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Padding(
                                ...
                                child: Text(
                                  '   Inventario: ${miles.format(stockValue)}',
                                  style: Theme.of(context)
                                      .textTheme
                                      .bodyMedium!
                                      .copyWith(
                                ...
                                Expanded(
                                  child: InkWell(
                                    ...
                                    child: Container(
                                      ...
                                      child: Center(
                                        child: Text(
                                          'BUYNOW'.translate(context: context),
                                          textAlign: TextAlign.center,
                                          style: Theme.of(context)
                                              .textTheme
                                              .titleMedium!
            ...
            child: Column(
              ...
              children: [
                Padding(
                  ...
                  child: Text(
                    'MORE_PRODUCT'.translate(context: context),
              ...
              children: <Widget>[
                Padding(
                  padding: const EdgeInsetsDirectional.only(
                      start: 12.0, end: 15.0, bottom: 10),
                  child: Text(
                    'You are looking for'.translate(context: context),
                    style: Theme.of(context).textTheme.titleMedium!.copyWith(
                          color: Theme.of(context).colorScheme.fontColor,
                          fontFamily: 'ubuntu',
                        ),
          ...
          child: Column(
            ...
            children: [
              SizedBox(
                width: deviceWidth,
                child: Text(
                  'Customer Questions'.translate(context: context),
                  style: TextStyle(
  ...
  Future<void> getProduct1() async {
    ...
    if (isNetworkAvail) {
      try {
        ...
        setSnackbar(
            'somethingMSg'.translate(context: navigatorKey.currentContext!),
            navigatorKey.currentContext!);
        if (mounted) {
          setState(
            () {
