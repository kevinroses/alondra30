...
class _MultiplePaymentMethodsState extends State<MultiplePaymentMethods> {
  ...
  void initState() {
    super.initState();
    _cargarCreditoDisponible();
  }

  Future<void> _cargarCreditoDisponible() async {
    if (_cargandoCredito) return;
    
    setState(() {
      _cargandoCredito = true;
    });
    
    try {
      // Obtener customer_erp_id del cliente seleccionado o del usuario actual
      final customerErpId = _obtenerCustomerErpId();
      
      if (customerErpId.isNotEmpty) {
        final result = await CartRepository.obtenerCreditoDisponible(customerErpId);
        if (!result['error']) {
          setState(() {
            _creditoDisponible = (result['data'] as num).toDouble();
          });
        }
      }
    } catch (e) {
      print('Error cargando crédito disponible: $e');
    } finally {
      setState(() {
        _cargandoCredito = false;
      });
    }
  }
  
  String _obtenerCustomerErpId() {
    // Primero intentar obtener del cliente seleccionado en el carrito
    final cartProvider = context.read<CartProvider>();
    
    // Si hay un cliente seleccionado en el buscador, usar su ID
    // Esto requiere acceso al widget de búsqueda o pasar el ID como parámetro
    // Por ahora, usamos el ID del usuario actual como fallback
    final userProvider = context.read<UserProvider>();
    return userProvider.customerErpId ?? '';
  }
  ...
  Widget _buildAddPaymentForm(CartProvider cartProvider) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // Mostrar crédito disponible si el método es crédito
        ...
        DropdownButtonFormField<String>(
          ...
          items: availableMethods.map((method) {
            ...
            if (method['id'] == 'credito') {
              isDisabled = _creditoDisponible <= 0;
            }
