...
class CartProvider extends ChangeNotifier {
  ...
  void setproVarIds(List<String> productVariantIdsValue) {
  ...
  addCartItem(SectionModel? item) {
    if (item != null) {
      ...
      // Recalcular precios totales al agregar item
      recalculateTotals();
  ...
  updateCartItem(String? id, String qty, int index, String vId) {
    ...
    if (i != -1) {
      ...
      // Recalcular precios totales al actualizar item
      recalculateTotals();
  ...
  void recalculateTotals() {
    ...
    for (var item in _cartList) {
      if (item.productList != null && item.productList!.isNotEmpty) {
        ...
        if (product.prVarientList != null && product.prVarientList!.isNotEmpty) {
          ...
          final quantity = int.tryParse(item.qty ?? '1') ?? 1;
          
          // Si hay descuento aplicado, usar precio con descuento
          if (item.descuentoPorcentaje != null && item.descuentoPorcentaje! > 0) {
            final precioConDescuento = precioOriginal * (1 - item.descuentoPorcentaje!);
            subtotal += precioConDescuento * quantity;
            descuentoTotal += (precioOriginal - precioConDescuento) * quantity;
          } else {
    ...
    // Actualizar precios con descuentos aplicados
    oriPrice = subtotal;
  ...
  void aplicarDescuentosIndividuals(List<Map<String, dynamic>> descuentos) {
    for (var item in _cartList) {
      ...
      final apiDiscount =
          (productDiscount['porcentaje_descuento'] as num).toDouble() / 100.0;
      final currentDiscount = (item.descuentoPorcentaje ?? 0.0) / 100.0;

      // Aplicar el mayor porcentaje entre API y descuento actual
      final mejorDescuento = apiDiscount > currentDiscount ? apiDiscount : currentDiscount;
      item.descuentoPorcentaje = mejorDescuento * 100; // Guardar como porcentaje
      
      // Calcular precio con descuento
      if (item.originalPrice != null && item.originalPrice! > 0) {
        item.precioConDescuento = item.originalPrice! * (1 - mejorDescuento);
      }
    }
  ...
  Future getUserCart(
    ...
    try {
      ...
      await CartRepository.fetchUserCart(parameter: parameter).then(
        (getData) {
          if (!getData['error']) {
            ...
            // Actualizar precios con los valores de la API
            oriPrice = double.tryParse(getData['sub_total']?.toString() ?? '0') ?? 0.0;
            deliveryCharge = double.tryParse(getData['delivery_charge']?.toString() ?? '0') ?? 0.0;
            totalPrice = oriPrice + deliveryCharge;
          } else {
            _cartList.clear();
            oriPrice = 0.0;
            deliveryCharge = 0.0;
      ...
      // Notificar incluso en caso de error
      notifyListeners();
    }
  }
    ...
    if (context.read<UserProvider>().userId == '') {
      ...
      if (proIds.isNotEmpty) {
        try {
          ...
          await CartRepository.fetchUserOfflineCart(parameter: parameter).then(
            ...
            if (!offlineCartData['error']) {
              ...
              // Calcular precios para el carrito offline
              recalculateTotals();
            }
            _isProgress = false;
            // Notificar a los listeners para actualizar la UI
        ...
        _cartList.clear();
        _isProgress = false;
        // Notificar cuando no hay productos
        notifyListeners();
      }
    } else {
      _cartList = [];
      _isProgress = false;
      // Notificar cuando hay usuario logueado
      notifyListeners();
    }
  }
    ...
    if (isNetworkAvail) {
      try {
        ...
        if (!error) {
          ...
          // No sobreescribir oriPrice con el precio individual
          // En su lugar, recalcular el total del carrito
          recalculateTotals();
          update();
  ...
  bool validarPagosMultiples(BuildContext context) {
    double totalPagado = paymentMethods.fold(
        0.0, (sum, method) => sum + (method['amount'] as double));
    double totalRequerido = totalPrice;
