...
class UserClientSearchProvider extends ChangeNotifier {
  ...
  void onTextChanged(String value, BuildContext context) {
    if (value.length >= 2) {
      ...
      _debounce.call(() => _performSearch(value, context));
    } else {
      _debounce.cancel();
      _isLoading = false;
      _showResults = false;
  ...
  Future<void> _performSearch(String query, BuildContext context) async {
    try {
      ...
      final userIdErp = userProvider.userIdErp?.toString() ?? '';
      final users = await UserClientSearchRepository.fetchUsersByName(
        name: query,
        userIdErp: userIdErp,
      );
      _users = users;
